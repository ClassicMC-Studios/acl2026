...
*/
/* 
    ADDED IMAGE AND RECT ROTATE
    ADDED ID GRABBER
    ADDED ROUND RECT
    ADDED FLIPPED IMAGES
*/
...
function image(img,x,y,w,h,opacity=1,flip=false){
        c.globalAlpha = opacity;
        c.save();
        if(flip){c.scale(-1, 1);c.drawImage(img,-x,y,w*-1,h);}
...
function roundRect(x,y,w,h,r,opacity = 1){
    c.globalAlpha = opacity;
    c.lineJoin = "round";
    c.lineCap = "round"; 
    c.beginPath();
...
function rectRotated(x,y,w,h,angle,opacity=1){
    c.globalAlpha = opacity;
    c.save();
...
function imageRotated(image,x,y,w,h,angle,opacity=1){
    c.globalAlpha = opacity;
    c.save();
    c.translate(x + w / 2, y + h / 2);
    c.rotate(angle * Math.PI / 180);
